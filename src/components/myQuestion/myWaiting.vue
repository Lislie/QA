    ...
    <div class='waitCheck'>
      <div class='a_lists' v-for='(item,index) in lists' ref='lists'>
        <h1 class='q_title textBig' @click='isShow(index)' v-show='show_a & index === activeIndex'>
          问题:{{item.question}}<span class='q_static'>待审核</span><i class='el-icon-minus up'></i></h1>
        <h1 class='q_title overHiden' @click='isHide(index)' v-show='index !== activeIndex || !show_a'>
          问题:{{item.question}}<i
          class='el-icon-plus down'></i></h1>
        <el-collapse-transition>
          <div v-show='show_a & index === activeIndex'>
            ...
            <div class='myAnswer'>
              <h2 class='a_top'><i class='lamp'></i>我的回答<span class='a_times'>{{item.createTime}}</span>
              </h2>
...
<script>
  export default {
    ...
    methods: {
      ...
      getDetail (n) {
        var _this = this
        this.$indexServer.nonChecked(n, _this.pageSize)
          .then((res) => {
            if (res.data.code == 200) {
              _this.lists = res.data.data
              _this.listsSize = res.data.total
              console.log(_this.listsSize+'kkk')
